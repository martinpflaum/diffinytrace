...
def gaussian_func2D(eval_points:torch.Tensor,
    ...
    Args:
        eval_points (torch.Tensor): Points where the Gaussian function is evaluated.
        x_range (tuple): Range of the target plane in the vertical direction.
        y_range (tuple): Range of the target plane in the horizontal direction.
        x_grid_size (int): Number of Gaussian points in the vertical direction.
        y_grid_size (int): Number of Gaussian points in the horizontal direction.
        sigma (float): Standard deviation of the Gaussian function.
    ...
    out1 = gaussian_func1D(eval_points1,x_range,x_grid_size,sigma,include_boundary)
    out2 = gaussian_func1D(eval_points2,y_range,y_grid_size,sigma,include_boundary)
    ...
    if summed is False:
        ...
        out = out1.reshape(x_grid_size,1,-1)*out2.reshape(1,y_grid_size,-1)
        return out
    return out1@out2.T
...
def calc_smooth_desired_irradiance(desired_irradiance_fun:Callable,
    ...
    with torch.no_grad():
        out = [desired_irradiance_fun(points)]
...
class GaussianSmoother():
    ...
    def __init__(self,
        ...
        self.discrete_desired_irradiance = desired_irradiance_fun(centers)
        
        
    
        self.smoothed_desired_irradiance = calc_smooth_desired_irradiance(desired_irradiance_fun,
                                   x_range,y_range,
                                   x_grid_size,
                                   y_grid_size,
                                   sigma=sigma,
...
def make_evaluation_function(optical_system:SequentialOpticalSystem,
    ...
    def evaluate():
        ...
        residual = raycounting.reshape(-1,2)-smoother.discrete_desired_irradiance.reshape(-1,2)
...
def make_merit_function(optical_system:SequentialOpticalSystem,
    ...
    def merit_function()->torch.Tensor:
        ...
        if use_desired_irradiance_smoothing:
            residual = smooth_irradiance.reshape(-1,2)-smoother.smoothed_desired_irradiance.reshape(-1,2)
        else:
            residual = smooth_irradiance.reshape(-1,2)-smoother.discrete_desired_irradiance.reshape(-1,2)
